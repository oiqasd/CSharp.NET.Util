...
namespace CSharp.Net.Standard.RedisManager
{
    public class RedisService
    {
        ...
        private static RedisServiceConfig _config;
        ...
        {
            if (config == null || string.IsNullOrEmpty(config.Address)) throw new KeyNotFoundException("redis config is not set");
            _config = config;
        ...
        private void Dispose(ConnectionMultiplexer redisClient) => redisClient.Dispose();

        #region 之前的写法，套用一下，后面不要用这些
        public void Set(string key, object obj, TimeSpan expiresIn)
        {
            _db.StringSet(key, Serialize(obj), expiresIn);
        }

        public void Set(string key, object obj, DateTime expiresAt)
        {
            TimeSpan span = expiresAt - DateTime.Now;
            _db.StringSet(key, Serialize(obj), span);
        }

        public void PrependItemToList(string key, string obj)
        {

            var inx = _db.ListLeftPush(key, obj);
            if (inx <= 0)
            {
                Console.WriteLine($"PrependItemToList count{inx}");
            }
        }
        ...
        public void PrependRangeToList(string key, List<string> values)
        {
            ...
            {
                var inx = _db.ListLeftPush(key, va);
                if (inx <= 0)
                {
                    Console.WriteLine($"PrependRangeToList count{inx} key:{va}");
                }
            }
        ...
        {
            return _db.StringBitCount(listId);
        }

        public void Remove(string key)
        {
            bool res = _db.KeyDelete(key);
            if (!res)
            {
                Console.WriteLine($"Remove key:{key} fail");
            }
        }
        ...
        public void RemoveAll(IEnumerable<string> keys)
        {
            ...
            foreach (var key in keys)
            {
                ...
                if (!res)
                {
                    Console.WriteLine($"RemoveAll key:{key} fail");
                }
            }
        ...
        static T Deserialize<T>(byte[] stream)
        {
            ...
            {
                T result = (T)binaryFormatter.Deserialize(ms);
                return result;
            }
        }
        ...
        /// <returns></returns>
        private Dictionary<string, T> ConvetDic<T>(HashEntry[] values)
        {
        ...
        {
            return _conn.GetDatabase(_config.DefaultDb);
        }
